...
function FeesPage() {
  ...
  const feesQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("fees")
        .select("id, name, amount, frequency, period_start, period_end, payment_date, applies_to_group_ids, section_id, kind")
        .order("name");
      if (error) throw error;
  ...
  const [form, setForm] = useState({
    name: "", amount: 0, frequency: "Monthly", period_start: "", period_end: "",
  ...
  const addFee = useMutation({
    ...
    onSuccess: () => {
      ...
      setForm({ name: "", amount: 0, frequency: "Monthly", period_start: "", period_end: "", kind: form.kind });
    ...
    <>
      ...
      <div className="space-y-6">
        <Card>
          ...
          <FeeTable rows={sectionFees.filter((f) => f.kind === "fee")} kind="fee" t={t} groups={groups} lang={lang} onDelete={(id) => delFee.mutate(id)} />
        </Card>
        ...
        <Card>
          ...
          <FeeTable rows={sectionFees.filter((f) => f.kind === "rate")} kind="rate" t={t} groups={groups} lang={lang} onDelete={(id) => delFee.mutate(id)} />
        </Card>
        ...
        <Card>
          ...
          <div className="mt-4 flex justify-end">
            <Button className="rounded-full" disabled={!activeSec || addFee.isPending} onClick={() => addFee.mutate()}>
              <Plus className="mr-1 h-4 w-4" />{t("add_fee")}
            </Button>
          </div>
        </Card>
      </div>
...
function FeeTable({ rows, kind, t, groups, lang, onDelete }: { rows: DBFee[]; kind: "fee" | "rate"; t: (k: string) => string; groups: { id: string; name: string }[]; lang: "es" | "en"; onDelete: (id: string) => void }) {
        ...
        <tbody>
          {rows.map((f) => (
            <tr key={f.id} className="border-t border-border hover:bg-muted/30">
              ...
              {kind === "fee" && <td className="px-2 py-2.5 text-muted-foreground">{f.period_start ? formatDate(f.period_start) : "—"} → {f.period_end ? formatDate(f.period_end) : "—"}</td>}
              <td className="px-2 py-2.5 text-muted-foreground">{!f.applies_to_group_ids || f.applies_to_group_ids.length === 0 ? "Todos" : f.applies_to_group_ids.map((id) => groups.find((g) => g.id === id)?.name).join(", ")}</td>
