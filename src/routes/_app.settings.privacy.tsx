...
const AI_MODULES = [
  { module: "Resúmenes en comunicación", on: true },
  { module: "Sugerencias en cuotas y pagos", on: true },
  { module: "Asistente en módulo médico", on: false },
  { module: "Generación de comunicados a familias", on: true },
  { module: "Análisis de rendimiento deportivo", on: false },
];
...
function PrivacyMockup() {
  ...
  const [aiState, setAiState] = useState(AI_MODULES);
            ...
            <tbody>
              {RETENTION.map((r) => (
                <tr key={r.area} className="border-t border-border">
                  <td className="px-3 py-2.5 font-medium">{r.area}</td>
                  <td className="px-3 py-2.5">{r.period}</td>
                  <td className="px-3 py-2.5 text-muted-foreground">{r.basis}</td>
                </tr>
              ))}
            </tbody>
          </table>
