...
const ROUTES: { area: string; path: string; role: string; status: Status; note?: string }[] = [
  ...
  { area: "Deportistas (filtros + ficha)", path: "/athletes", role: "admin · manager · technical · medical", status: "ok" },
  { area: "Calendario (entrenos, partidos, reuniones)", path: "/calendar", role: "admin · manager · technical", status: "ok" },
  { area: "Asistencia y disponibilidad", path: "/attendance", role: "admin · manager · technical", status: "ok", note: "Mockup demo, listo para piloto" },
  { area: "Comunicación segmentada", path: "/communication", role: "admin · manager · technical · medical", status: "ok" },
  ...
  { area: "Restricciones y lesiones", path: "/medical/restrictions", role: "medical · admin", status: "ok", note: "Datos representativos" },
  { area: "Privacidad y seguridad", path: "/settings/privacy", role: "admin · sysadmin", status: "ok" },
  { area: "Equipo y permisos", path: "/settings/team", role: "admin", status: "ok" },
  { area: "Onboarding club", path: "/onboarding", role: "admin", status: "partial", note: "Wizard funcional, pendiente personalización por deporte" },
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-4">
          <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Listos</div>
          <div className="mt-1 text-2xl font-bold text-emerald-600">{ok}</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Parciales</div>
          <div className="mt-1 text-2xl font-bold text-amber-600">{partial}</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Total módulos auditados</div>
          <div className="mt-1 text-2xl font-bold">{ROUTES.length}</div>
        </Card>
      </div>

      <Card className="mt-6 p-0">
        <div className="border-b border-border px-5 py-3 text-sm font-semibold">Recorrido por rol</div>
              ...
              {ROUTES.map((r) => {
                ...
                return (
                  <tr key={r.path} className="border-t border-border">
                    ...
                    <td className="px-3 py-2.5"><Link to={r.path} className="text-xs text-primary hover:underline">{r.path}</Link></td>
      ...
      <Card className="mt-6 p-6">
        ...
        <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
          ...
          <li>• La IA no realiza diagnóstico médico; sólo organiza información clínico-administrativa.</li>
          <li>• Acceso a datos sensibles auditado y visible en <Link to="/settings/privacy" className="text-primary hover:underline">/settings/privacy</Link>.</li>
