...
function PaymentsPage() {
  ...
  const paymentsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("payments")
        .select("id, athlete_id, subscription, section_id, category_id, amount, status, payment_date")
  ...
  const subs = Array.from(new Set(payments.map((p) => p.subscription).filter((s): s is string => !!s)));
  ...
  const exportCsv = () => {
    ...
    const rows = filtered.map((p) => {
      ...
      return [`${a?.first_name ?? ""} ${a?.last_name ?? ""}`, p.subscription ?? "", s?.name ?? "", p.amount, p.status, p.payment_date ?? ""];
    });
              ...
              {paged.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-t border-border hover:bg-muted/30">
                    <td className="px-5 py-3 font-medium">{a ? formatName(a.first_name, a.last_name) : "—"}</td>
                    <td className="px-5 py-3">{p.subscription ?? "—"}</td>
                    <td className="px-5 py-3 text-muted-foreground">{sections.find((s) => s.id === p.section_id)?.name}</td>
                    <td className="px-5 py-3 text-muted-foreground">{categories.find((c) => c.id === p.category_id)?.name}</td>
                    ...
                    <td className="px-5 py-3 text-muted-foreground tabular-nums">{p.payment_date ? formatDate(p.payment_date) : "—"}</td>
                    <td className="px-5 py-3 text-right whitespace-nowrap">
                      <Button size="sm" variant="ghost" onClick={() => openInvoice(p.id)}>{t("view_invoice_pdf")}</Button>
                      {p.status !== "Paid" && (
                        <Button size="sm" variant="ghost" className="text-success" onClick={() => setStatus.mutate({ id: p.id, status: "Paid" })}>{t("mark_paid")}</Button>
                      )}
                    </td>
                  </tr>
                );
